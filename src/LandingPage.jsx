...
const [walletAvailableEarnings, setWalletAvailableEarnings] = useState(0);
const [walletPendingEarnings, setWalletPendingEarnings] = useState(0);

const loadWalletActivity = useCallback(async () => {
  if (!user?.id) {
    setWalletTransactions([]);
    setWalletAvailableEarnings(0);
    setWalletPendingEarnings(0);
    return;
  }
  ...
  if (transactionsError) {
    console.error("WALLET TRANSACTIONS LOAD ERROR:", transactionsError);
  } else {
  ...
  if (earningsError) {
    console.error("CALL EARNINGS LOAD ERROR:", earningsError);
    return;
  }
  ...
  const available = (earnings || [])
    ...
    .reduce(
      (total, earning) => total + Number(earning.host_amount || 0),
      0
    );
  ...
  setWalletAvailableEarnings(available);
  setWalletPendingEarnings(pending);
}, [user?.id]);
 ...
 fontSize: 13,
    ...
    <div style={headerActions}>
      ...
      <button
        ...
        aria-label="Cirql Balance"
        title="Cirql Balance"
        style={walletButton(theme)}
      >
        <Wallet size={14} strokeWidth={2.2} />
        <span style={walletBalanceText(theme)}>
      ...
      {incomingCall && (
        <div
          style={{
            ...
            boxShadow:
              "0 18px 55px rgba(0,0,0,.45), 0 0 0 1px rgba(255,255,255,.04)",
            color: "#fff",
            overflow: "hidden",
          }}
        >
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  border: "2px solid #22c55e",
                  animation: "upcallIncomingPulse 1.5s ease-out infinite",
                }}
              />
              ...
              <div
                style={{
                  ...
                  background: "rgba(255,255,255,.08)",
                  border: "2px solid #22c55e",
                  boxShadow: "0 0 18px rgba(34,197,94,.28)",
                }}
              >
              ...
              >
                <span
                  style={{
                    ...
                    animation: "upcallIncomingDot 1s ease-in-out infinite",
                  }}
                />
              ...
              onClick={async () => {
                ...
                const { error } = await supabase
                  ...
                  .insert({
                    ...
                    payload: {
                      call_id:
                        incomingCall.payload?.call_id || null,
                      call_type:
                ...
                const callId = incomingCall.payload?.call_id;
